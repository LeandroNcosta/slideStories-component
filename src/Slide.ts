...
export default class Slide {
  ...
  auto(time: number) {
    ...
    if (this.time && this.thumbItems) this.thumbItems[this.index].style.animationDuration = `${time}ms`

  }
  pause() {
    const slideAtual = this.slides[this.index]

    // para entender se esta segurando a tela ou n
  ...
  continue() {
    ...
    if (this.paused) {
      ...
      if (slideAtual instanceof HTMLVideoElement) slideAtual.play()
    }
  ...
  autovideo(video: HTMLVideoElement) {
    ...
    video.addEventListener('playing', () => {
      const duration = video.duration * 1000  // transforma s para ms
      if (firstPlay) this.auto(duration)
      firstPlay = false
    })
  }
  //
